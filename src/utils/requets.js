...
//  构造参数 传入一些配置 和defaults一样
  ...
  transformResponse: [function (data) {
    //   data就是后端响应回来的字符串
    // JSON.parse()  这里是修改后端返回回来的数据,因为从后端返回的是JSON字符串 我们先处理成JSON对象 再返回
...
instance.interceptors.request.use(function (config) {
  if (store.state.userToken.token) {
    config.headers.Authorization = `Bearer ${store.state.userToken.token}` // 将token 统一注入到headers中
  }
  return config // 返回配置
  // 不用 if else的用法
...
instance.interceptors.response.use(function (response) {
  ...
  try {
    ...
    return response.data // 如果失败返回,里面没有data数据的话, data就返回一层
  }
},
async function (error) {
  ...
  if (error.response && error.response.status === 401) {
    const path = {
      ...
      query: {
        // 需要传递的query参数
        redirect: router.currentRoute.fullpath // 表示登录页需要跳转的地址
      }
    ...
    if (store.state.user.refresh_token) {
      ...
      try {
        const result = await axios({
          method: 'put', // 请求类型
          url: 'http://ttapi.research.itcast.cn/app/v1_0/authorizations', // 请求地址
        ...
        store.commit('updateUserToken', {
          user: {
            token: result.data.data.token, // 获取最新的token
            refresh_token: store.state.user.refresh_token // 还是原来的refresh_token, 有效期还是14天
